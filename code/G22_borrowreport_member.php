...
<body>
	...
	<div id="up">
		<div id="search">
			<input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for name.." ><br/>
		</div>
	</div>
	<br/>
	...
	<table id="table">
		<tr>
			...
			<th style="width: 8%;">Number of times borrow books</th>
		</tr>
		<?php
			$data="SELECT *, COUNT(borrow.member_id) FROM borrow RIGHT JOIN member on member.member_id=borrow.member_id 
			...
			if(mysqli_num_rows($result)>0) //if there are data
			{
				while($row=mysqli_fetch_array($result))
				{
					...
					<tr>
						...
						<td><?php echo $row["COUNT(borrow.member_id)"]; ?></td>
					</tr>
					<?php
				}
			}
