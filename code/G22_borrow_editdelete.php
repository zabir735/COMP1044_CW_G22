...
<body>
	...
	<div id="up">
		<div id="search">
			<input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for borrow status.." title="Type in a name">
		</div>
	</div>
	<br/>
			...
			if(mysqli_num_rows($result)>0) //if there are data
			{
				while($row=mysqli_fetch_array($result))
				{
					...
					<tr>
						...
						<td>
							<div id="edit">
								<a href="G22_borrow_edit.php?edit&id=<?php echo $row["borrow_details_id"];?>">
								<button>
						...
						<td>
							<div id="dlt">
								<a href="G22_borrow_editdelete.php?del&id=<?php echo $row["borrow_details_id"];?>" onclick="return confirmation();">
								<button>
...
<?php
	...
	{
		$id = $_REQUEST["id"]; 
		mysqli_query($connect, "DELETE FROM borrowdetails WHERE borrow_details_id=$id");
